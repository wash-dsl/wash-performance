...
if [ $version == "sphexa" ]
then
    jobname=$(sbatch oneperfvmpi$nodes.sbatch $sourcedir/$version $n_particles $timestep $numthreads $nodes)
else
    jobname=$(sbatch oneperfvmpi$nodes.sbatch $sourcedir/$version $n_particles $timestep $numthreads $nodes)
fi
...
mkdir -p out/viper_$version
echo "slurm-$jobname.out" >> out/viper_$version/$2-$3-$4-$5-no-out-jobs.txt
echo $jobname