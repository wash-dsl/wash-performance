...
step_count=100
exa_steps=$(($step_count + 5))
echo "$exa_steps"
echo ""
printf -v sedov_num "%04d" $(( step_count - 1 ))

# Run WaSH Sedov
time_output=$(time -p (./wash/build/sedov $particle_count $step_count) 2>&1)
real_time=$(echo "$time_output" | grep "real" | awk '{print $2}')
printf "WaSH Sedov execution time: $real_time\n"

echo "Generating WaSH Sedov graphs"
output=$(python3 ./wash/src/examples/sedov_solution/compare_solutions_wash.py out/sedov/sedov.$sedov_num.h5)
echo "$output"
...
case $sphexa in
    1) 
        ...
        time_output=$(time -p (./sph-exa-build/main/src/sphexa/sphexa --quiet --init sedov -n $particle_count -s $exa_steps -w 2 -f x,y,z,rho,p,vx,vy,vz) 2>&1)
        real_time=$(echo "$time_output" | grep "real" | awk '{print $2}')
        echo "SPH-EXA Sedov execution time:"
        echo "$real_time"
        
        echo "Generating SPH-EXA Sedov graphs"
        python3 ./wash/src/examples/sedov_solution/compare_solutions.py --time $t dump_sedov.h5
