...
if [ $version == "sphexa" ]
then
    jobname=$(sbatch nohoneperfmpi$nodes.sbatch $sourcedir/$version $n_particles $timestep $numthreads $nodes)
else
    jobname=$(sbatch nohoneperfmpi$nodes.sbatch $sourcedir/$version $n_particles $timestep $numthreads $nodes)
fi
...
mkdir -p out/noh_$version
echo "slurm-$jobname.out" >> out/noh_$version/$2-$3-$4-$5-no-out-jobs.txt
echo $jobname